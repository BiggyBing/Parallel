...
class CNN(Parallel_base):
    ...
    def __init__(self, batch_size = 50, learning_rate = 0.0001, train_type = 'Parallel', iteration = 1000):
    ...
    def fit(self, dataset):
        ...
        layer1 = create_new_conv_layer(x_shaped,self.w[:2],1, 32, [5, 5], [2, 2], name='layer1')
        layer2 = create_new_conv_layer(layer1,self.w[2:4], 32, 64, [5, 5], [2, 2], name='layer2')

        flattened = tf.reshape(layer2, [-1, 7 * 7 * 64])
        
        # setup some weights and bias values for this layer, then activate with ReLU
        wd1 = tf.Variable(self.w[4].reshape(7*7*64,1000), name='wd1')
        bd1 = tf.Variable(self.w[5], name='bd1')
        dense_layer1 = tf.matmul(flattened, wd1) + bd1
        dense_layer1 = tf.nn.relu(dense_layer1)

        # another layer with softmax activations
        wd2 = tf.Variable(self.w[6].reshape(1000,10), name='wd2')
